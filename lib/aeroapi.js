...
/**
 * Kies de vlucht waarvan scheduled_out (of _off) op de gegeven UTC-datum valt.
...
function pickFlightOnDate(flights, dateISO) {
  ...
  for (const f of flights) {
    ...
    if (!sched) continue;
    if (String(sched).slice(0, 10) === dateISO) return f;
  }
  return null;
}
...
async function getFlightStatus(icaoIdent, dateISO) {
  ...
  return {
    ...
    depSource: dep ? dep.source : null,  // 'actual'|'estimated'|'scheduled'
    arrTime: arr ? arr.time : null,
    arrSource: arr ? arr.source : null,
    status: label,                        // 'On schedule'|'Delayed'|'Unknown'
    delayMin,
