...
async function getTracking(awbWithDash) {
  ...
  if (res.status === 404 || (res.body && (res.body.code === 4031 || res.body.code === 4032))) {
    // Niet geregistreerd → eenmalig aanmaken
    await tmFetch('/air_waybill/create', {
      method: 'POST',
      body: { awb_number: awbWithDash },
    });
    // Direct opnieuw proberen (data is meestal nog leeg in eerste minuten)
